...
exports.StudentsDegrees = void 0;
// packages
const mongoose_1 = __importDefault(require("mongoose"));
const constants_1 = require("../core/constants");
const studentDegreeSchema = new mongoose_1.default.Schema({
    student_name: {
        type: String,
        required: true,
    ...
    section: {
        ...
        required: true,
        enum: {
            values: Object.values(constants_1.Sections),
            message: "Student status value must be `SCIENCE_MATH`, `SCIENCE_BIOLOGY` or `LITERATURE`",
        },
    },
    fail_subjects_count: {
        type: Number,
    ...
    student_status: {
        ...
        required: true,
        enum: {
            values: Object.values(constants_1.StudentStatus),
...
studentDegreeSchema.pre("validate", function (next) {
    ...
    // check Math subjects
    if (this.subjects) {
        if (this.section === constants_1.Sections.SCIENCE_MATH) {
            if (!this.subjects.arabic ||
            ...
            if (!this.subjects.psychology && !this.subjects.philosophy) {
                this.invalidate("subjects", "you should set one of these subjects: psychology, philosophy");
            }
            if (this.subjects.psychology && this.subjects.philosophy) {
                this.invalidate("subjects", "you should set only one literature subject: psychology or philosophy");
            }
            if (this.subjects.biology ||
                this.subjects.history ||
                this.subjects.geography) {
                this.invalidate("subjects", "you cannot set these subjects: biology, history, geography");
            }
        }
        // check Biology subjects
        if (this.section === constants_1.Sections.SCIENCE_BIOLOGY) {
            if (!this.subjects.arabic ||
                !this.subjects.foreign_1 ||
                !this.subjects.foreign_2 ||
        ...
        if (this.section === constants_1.Sections.LITERATURE) {
            if (!this.subjects.arabic ||
                !this.subjects.foreign_1 ||
                !this.subjects.foreign_2 ||
                !this.subjects.history ||
                ...
                this.invalidate("subjects", "you should set student section's subjects");
            }
            if (this.subjects.math ||
                this.subjects.biology ||
                this.subjects.chemistry ||
                this.subjects.physics) {
                this.invalidate("subjects", "you cannot set these subjects: biology, math, chemistry, physics");
            }
        }
    }
    next();
});
exports.StudentsDegrees = mongoose_1.default.model(constants_1.StudentsDegreesCollection, studentDegreeSchema);
